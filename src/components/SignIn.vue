<template>
  <div class="flex flex-wrap gap-4">
    <div class="flex flex-col justify-between gap-4">
      <Button
        class="p-4 cursor-pointer flex gap-3 items-center justify-center"
        @mousedown="signIn"
      >
        <AntDesignGoogleCircleFilled class="size-6" /> Sign in with Google
      </Button>
      <p class="text-secondary-foreground w-min opacity-30">StockT</p>
    </div>
...
.bg::before {
  ...
  width: 100%;
  height: 100%;
  box-shadow: inset 0 0 1rem 1rem hsl(var(--background));
  position: absolute;
}
.dark .bg,
.dark .bg::before {
